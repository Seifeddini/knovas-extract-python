...
7. **No telemetry, no content in logs, no content in warnings.** Zero outbound metrics. Logging is at INFO/WARNING and emits only counts/sizes/MIMEs — never document content. `result.warnings` follows the same rule: entries are either counted aggregates (`"metadata: 3 values truncated"`, `"markdown: 2 <script> tags stripped"`) or descriptive of a fixed condition — **never** substrings of the input. This guarantee extends to the new surfaces added in `spec_version = 1.2.0`:
